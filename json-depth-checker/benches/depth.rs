...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("null", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&null))
    });
    c.bench_function("true", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&bool_true))
    });
    c.bench_function("false", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&bool_false))
    });
    c.bench_function("integer", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&integer))
    });
    c.bench_function("float", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&float))
    });
    c.bench_function("string", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&string))
    });
    c.bench_function("object", |b| {
        b.iter(|| should_flatten_from_unchecked_slice(&object))
    });
